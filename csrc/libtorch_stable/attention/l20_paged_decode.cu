...
void l20_paged_decode_split_out_cuda(
    ...
    torch::stable::Tensor& partial_output,
    torch::stable::Tensor& partial_max,
    torch::stable::Tensor& partial_sum,
    torch::stable::Tensor& output,
    int64_t max_seq_len,
    int64_t split_size) {
  STD_TORCH_CHECK(query.device().is_cuda(), "query must be CUDA");
  STD_TORCH_CHECK(query.scalar_type() == torch::headeronly::ScalarType::Half,
                  "FP16 only");
  STD_TORCH_CHECK(query.dim() == 3 && query.size(2) == kHeadDim,
                  "Q must be [B,H,128]");
  STD_TORCH_CHECK(key_cache.dim() == 4 && key_cache.size(3) == kHeadDim,
                  "NHD cache only");
  ...
  STD_TORCH_CHECK(err == cudaSuccess,
                  "l20_paged_decode_split_out failed: ",
